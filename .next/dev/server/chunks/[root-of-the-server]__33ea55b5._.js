...
async function POST(req) {
    try {
        ...
        // 1. Find user
        const user = db.prepare('SELECT * FROM users WHERE access_key = ?').get(access_key);
        if (!user) {
        ...
        // 3. Device check
        const devices = db.prepare("SELECT * FROM devices WHERE user_id = ? AND status = 'active'").all(user.id);
        ...
        // 4. Create session
        const token = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$auth$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["createToken"])({
            user_id: user.id,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__33ea55b5._.js.map